...
if __name__ == '__main__':
    ...
    for temp_agg in temp_agg_list:
        ...
        obsv_res = df_obsc_cross_val.resample(temp_agg).sum()
    ...
    # In[363]:
        ...
        df_pws_raw = pd.DataFrame(columns=df_obsc_cross_val.columns,
                                             index=index_to_krige_cmn,
                                             data=np.nan)
        df_pws_pwspyqc= pd.DataFrame(columns=df_obsc_cross_val.columns,
                                             index=index_to_krige_cmn,
                                             data=np.nan)
        df_pws_pwsqc = pd.DataFrame(columns=df_obsc_cross_val.columns,
                                             index=index_to_krige_cmn,
                                             data=np.nan)
        df_pws_intense = pd.DataFrame(columns=df_obsc_cross_val.columns,
                                             index=index_to_krige_cmn,
                                             data=np.nan)
        df_pws_radar = pd.DataFrame(columns=df_obsc_cross_val.columns,
                                             index=index_to_krige_cmn,
                                             data=np.nan)
    ...
    #===========================================================================
        ...
        for ix, evt_index in tqdm.tqdm(enumerate(index_to_krige_cmn)):
            ...
            #         plt.tight_layout()
            #         plt.savefig(r'X:\staff\elhachem\2022_02_01_OpenSense\interpolation'
            #                     r'\cross_val_loc_%d.png' % ix, bbox_inches='tight')
            #         plt.close()
                #### RAW
                OK_raw = OKpy(
                        xpws_raw,
